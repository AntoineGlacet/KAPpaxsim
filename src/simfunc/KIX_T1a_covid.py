...
# I replaced display by print (just for displaying df_param)
# this is mostly when we call loops with simulation runs
# it's nice to display df_param for each run
...
def KIX_T1_covid_arrivals_v2(
    ...
    # DEBUG
    global df_result
    global dct_processes
    global dct_resources
    global df_Pax_flight
    global list_flight
    global plt_queue_length
    global plt_hist_wait_time
    ...
    kpi_queue_length = [
        min(
            heapq.nlargest(
                int(len(list_kpi_queue_length[i]) / 90) + 1, list_kpi_queue_length[i]
            )
        )
        for i in range(n_graph)
    ]
    kpi_wait_time = [
        min(
            heapq.nlargest(
                int(len(list_kpi_wait_time[i]) / 90) + 1, list_kpi_wait_time[i]
            )
        )
        for i in range(n_graph)
    ]
